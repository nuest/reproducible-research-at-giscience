...
## [WIP] Figure: Alluvial plot of reproducibility assessment results

Based on package [`ggalluvial`](https://corybrunson.github.io/ggalluvial/).

This figure does not include the categories `preprocessing` and `computational environment` because they were deemed as quite hard to assess, and have a large fraction of missing values.
...
**WITHOUT PREPROCESSING AND COMP ENV, WITHOUT NAs:**
...
ggplot(data = papers_wide,
  ...
  scale_x_discrete(limits = c("Input data",
                              "Methods/\nAnalysis/\nProcessing",
                              "Results"),
                   expand = c(.1, 0)
                   ) +
  xlab("Category") +
  colorspace::scale_fill_discrete_qualitative(aesthetics = "fill") +
  geom_alluvium(aes(fill = code), width = 1/3) +
...
**Drop preprocessing and comp env, keep NAs**
...
**Dropped NAs, and re-grouped to "below minimum standard" and "better (1-2)", no 3s anyway**
...
papers_wide <- paper_evaluation_wo_conceptual %>%
  ...
  drop_na() %>%
  mutate_if(is.factor, forcats::fct_rev) %>%
  mutate_if(is.factor, ~ forcats::fct_collapse(., `0` = c("0"), `1, 2`= c("1","2","3"))) %>%
  group_by(`input data`,
           preprocessing,
           `method/analysis/processing`,
           `computational environment`,
...
**Dropped NAs, group levels, only 3 categories**

```{r fig_alluvial_5}
papers_wide_4 <- paper_evaluation_wo_conceptual %>%
  #mutate_if(is.factor, forcats::fct_explicit_na, na_level = "NA") %>%
  drop_na() %>%
  mutate_if(is.factor, forcats::fct_rev) %>%
  mutate_if(is.factor, ~ forcats::fct_collapse(., `0` = c("0"), `1, 2`= c("1","2","3"))) %>%
...
**Dropped NAs, without preprocessing, horizontal:**
