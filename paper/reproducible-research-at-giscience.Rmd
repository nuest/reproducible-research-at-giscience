...
The past two decades have seen the imperative of Open Science gain increased momentum across scientific disciplines, at least partially prompted by the increasing costs of using proprietary software and subscribing to scientific journals. 
One important element, especially for the computational and natural sciences, is transparency and availability of data, methods, and results, to enable reproducibility (REF MANUFO2017 10.1038/s41562-016-0021).
...
The related concept of replication, i.e., the confirmation of insights gained from a scientific study, is of crucial importance to scientific progress, yet also frequently a challenge for interested readers of a published study.
...
With reproducibility now being a recognized topic in the call for papers, it is time to take stock and identify possible action. 
...
We believe that GIScience as a discipline would greatly benefit from more studies that reproduce and replicate other studies, similar to other disciplines that are recognizing the value of replication for innovating theory \cite{nosek_what_2020}, and argue that such a replication study is not lacking innovation but is a prerequisite for innovating community practice.
...
This is partly because of no obvious subset exists, such as the nominees for best papers as in the case of the AGILE conference series, but also because we aimed to work with a larger dataset for potentially more informative results.
...
However, the dividing line between data preprocessing and processing (i.e., the main analysis) proved to be often vague, and several times, assessors disagreed whether the preprocessing criterion should be assigned _NA_, and if not whether _Unavailable_ or _Documented_ (`0` or `1`, respectively).
...
Figure&nbsp;\@ref(fig:alluvial-diagramme) shows an alluvial diagramme of all scores, i.e., combinations of criteria values, of those `r sum(papers_wo_na_wo_prepr$n)`&nbsp;papers without any _NA_ criterion.
...
The diagramme illustrates how groups of papers with similar properties 'flow' through the different criteria
Three major groups, who represent `r papers_wo_na_wo_prepr %>% filter(n > 9) %>% .$n %>% sum()`&nbsp; of the papers (`r round(papers_wo_na_wo_prepr %>% filter(n > 9) %>% .$n %>% sum() / sum(papers_wo_na_wo_prepr$n), 2) * 100`%) included in the figure, become visible as broad bands.
...
In summary, not a single paper can reach the required levels for an immediate reproduction by ensuring that all required pieces are _Available_ (level&nbsp;`2`), not even considering the further challenges around documentation if availability is given (e.g.,&nbsp;\cite{nust_improving_2020}).
An investigation of yearly scores to track possible developments over time do not show any clear trend, i.e., there is little change in reproducibility over the study period. 
...
The overall objective should be to make a study as reproducible as possible before publication.
We argue that is that for most currently published studies, _Available_ would have been possible and feasible with reasonable efforts.  
...
Although the replication was successful, the process was again labor-intensive, making it problematic to scale it up to include several years of several popular journals, for example. 
Further, despite our best efforts for transparency, the process is inherently subjective, and a different set of investigators might score papers differently even when using this study's rubric.
While natural language processing techniques have made great progress in the past decades, an automated assessment of a paper's reproducibility seems still out-of-reach, unless at least some important information is included as machine-readable metadata. 
...
The published work and contributing community of the two conferences are similar enough for a comparison, yet their organization (setup, process, geographic focus) differ too much for a simplistic ranking.
...
AGILE, by comparison, has a European focus, and, although the conference location moves every year, accessibility is more homogeneous. 
This likely translates into a less fluctuating and less geographically diverse audience at AGILE. Clearly, these observations will need a reassessment in several years to evalute the impact of both conferences going full online in 2020/21 because of the travel and activity restrictions due to the COVID-19 pandemic.
...
However, this study does not intend to make judgments on AGILE vs. GIScience conference quality, nor to question the papers' scientific soundness or elevance, since they were accepted for publication at a reputable conference.
...
A successfull _reproducibility turn_ would not mean that every reprodudicible paper would be reproduced, nor would this be necessary. 
But at least for influential, i.e, highly cited papers, a validation of their applicability and transferability to other study areas should be possible.
...
There is currently a lot of activity in the GIScience discipline to address reproducibility and replicability of geospatial research. 
