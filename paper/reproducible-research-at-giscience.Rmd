...
We have previously referred to the minimum standard for reproduction. Level 1 is minimum acceptable standard if a paper is examined under the prism of "preproducibility" (reproduction by reading only) \cite{stark_2018}. However, considering that minimum, as noted above, actual reproduction is totally unpredictable since there is no guarantee of availability of data and methods. Fig 2 shows the distribution of papers by levels through all criteria except _Preprocessing_. The largest stream is for papers with "1101", which just barely meet the minimum standard for read-only reproduction. Note that the second largest stream is "0101", which seriously questions the reporting of input data in a paper.

For practical reproduction, the minimum standard bar for hand-on reproduction is set to 2, ensuring that all required pieces are available. Looking at Fig 2, only a small fraction of papers reaches level 2 (none for _Computational Environment_), which most likely will be successfully reproduced. Nevertheless, level 2 is necessary but not sufficient to carry out "always" a successful reproduction, since access to data and code say nothing about the quality and documentation of such resources (Daniel: one of your recent works can be cited here?).
